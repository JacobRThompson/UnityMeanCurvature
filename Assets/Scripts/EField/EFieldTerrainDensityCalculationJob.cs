...
namespace MarchingCubes.EField
{
    ...
    [BurstCompile]
    struct EFieldTerrainDensityCalculationJob : IDensityCalculationJob
    {
        /// <summary> The output densities </summary>
        [WriteOnly] private NativeArray<float> densities;

        /// <summary> The sampling point's offset </summary>
        [ReadOnly] public int3 offset;
        
        /// <summary> The number of subdivisions in this chunk </summary>
        [ReadOnly] public int chunkSubdivisions;

        /// <summary> base size of each chunk </summary>
        [ReadOnly] public int chunkBaseSize;

        /// <summary> The procedural terrain generation settings </summary>
        [ReadOnly] public EFieldTerrainSettings eFieldTerrainSettings;

        /// <summary> The chunk's density field </summary>
        public NativeArray<float> Densities { get => densities; set => densities = value; }

        /// <summary>The execute method required for Unity's IJobParallelFor job type</summary>
        /// <param name="index">The iteration index provided by Unity's Job System</param>
        public void Execute(int index)
        {
            float worldPositionX = index / (chunkSubdivisions * chunkSubdivisions) + offset.x;
            float worldPositionY = index / chunkSubdivisions % chunkSubdivisions + offset.y;
            float worldPositionZ = index % chunkSubdivisions + offset.z;

            float density = CalculateDensity(worldPositionX, worldPositionY, worldPositionZ);
            densities[index] = math.clamp(density, -1, 1);
        }

        /// <summary> Calculates the density at the world-space position </summary>
